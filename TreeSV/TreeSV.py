...
def dftt(service, cur_node, parent):
    ...
    while(count > 0):
        ...
        for child in children_of_cur_node:             
            ...
            if((count != len(children_of_cur_node)) and (glbl_root['id'] != cur_node['id'])): 
                ttr += parent + ',' + cur_node['name'] + ',' + dftt(service, child, parent + ',' + cur_node['name'])
                          
            else:
                if (parent == ''):
                    ttr += cur_node['name'] + ',' + dftt(service, child, cur_node['name'])
                else:
                    ttr += cur_node['name'] + ',' + dftt(service, child, parent + ',' + cur_node['name'])
