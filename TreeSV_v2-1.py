...
def dftt(service, cur_node, parent):
    ...
    while(count > 0):
        ...
        for child in children_of_cur_node:
            ...
            if((parent['name'] != desired_directory) and (count != len(children_of_cur_node))):
                ttr += desired_directory + ',' + parent['name'] + ',' + cur_node['name'] + ',' + dftt(service, child, cur_node)
                        
            
            elif((count != len(children_of_cur_node)) and (cur_node['name'] != parent['name'])):   
                ttr += parent['name'] + ',' + cur_node['name'] + ',' + dftt(service, child, cur_node)
            
            else:
                ttr += cur_node['name'] + ',' + dftt(service, child, cur_node)
...
def main():
    ...
    output_string = (dftt(service, glbl_root, glbl_root))
